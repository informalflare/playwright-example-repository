...
function runTests() {
  echo "RunTests"
      if argListContains --update; then
        ...
        printf "Running visual tests might take some time \n"
        docker run -v "$PWD":/folder --network=host -w /folder -it --rm --ipc=host $DOCKER_IMAGE  /bin/bash -c "npm ci && npx playwright install && npx playwright test --config ui/ci-cd.config.ts"
      fi
}
