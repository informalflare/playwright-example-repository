...
class OutputEl extends BaseElement {
    ...
    async getData():Promise<UserFormData>{
        ...
        if(await this.email.isVisible()){ data.email = await this.getEmail()}
        if(await this.curAddress.isVisible()){ data.curAdr = await this.getCurAdr()}
        if(await this.perAddress.isVisible()){ data.permAdr = await this.getPermAdr()}
        return data;
    }
